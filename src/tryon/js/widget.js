const oicWidget = (function() {
	...
	let $modelFigurePreview = document.querySelector('.oic-model__figure img');
	...
	function _init(options) {
		if (options.capturePage) {
			...
			document.querySelector('.oic-nav__prev').addEventListener('click', () => {
				currentModelInView = 0;
				$modelFigurePreview.setAttribute('src', '/public/img/widget/profile.jpg');
			});
			document.querySelector('.oic-nav__next').addEventListener('click', () => {
	...
	function _getFrames(frameId) {
		...
		request.onreadystatechange = () => {
			if (request.readyState === 4 && request.status === 200) {
				...
				const data = JSON.parse(request.responseText);
				console.log(data);
				$previewFrame.setAttribute('src', data.frameUrl);
				$previewFrame.style.left = `${data.left}px`;
				$previewFrame.style.top = `${data.top}px`;
				$previewFrame.style.width = `${data.width}px`;
				$previewFrame.style.transform = `perspective(200px) rotateX(${data.deviationX}deg) rotateY(${
					data.deviationY
				}deg) rotateZ(${data.deviationZ}deg)`;
			}
		};
		request.onerror = () => {
			alert('Something went wrong');
